...
def texttime_to_timestamp(timee: str):
    ...
    timea = datetime.datetime(
        now.year, now.month, now.day, int(t[0]) + 6, int(t[1]), 0, 0
    )
...
class Bloopers(commands.Cog):
    ...
    async def blooper(self, ctx: commands.Context):
        if ctx.subcommand_passed is None:
            return ctx.reply(
                content="{ctx.subcommand_passed} is not part of the bloopers group.",
                delete_after=60,
    ...
    @blooper.command(
        breif="Reports a blooper!",
        description="Reports a blooper to the current channel!",
    )
    async def report(self, ctx: commands.Context, time: str, *description: str):
        ...
        if not istime:
            return await ctx.reply(
                content="Please make sure to provide a valid time!",
                delete_after=60,
                mention_author=False,
            )
